...
public class RebalancePushImpl extends RebalanceImpl {
    ...
    private boolean unlockDelay(final MessageQueue mq, final ProcessQueue pq) {
        if (pq.hasTempMessage()) { // 若该顺序消费队列，有缓存消息未被消费完，则20s后请求Broker执行队列解锁
            log.info("[{}]unlockDelay, begin {} ", mq.hashCode(), mq);
            this.defaultMQPushConsumerImpl.getmQClientFactory().getScheduledExecutorService().schedule(new Runnable() {
                @Override
                public void run() { // 20s后执行该unlock方法，到broker上解锁该order消息队列
                    log.info("[{}]unlockDelay, execute at once {}", mq.hashCode(), mq);
                    RebalancePushImpl.this.unlock(mq, true);
                }
            }, UNLOCK_DELAY_TIME_MILLS, TimeUnit.MILLISECONDS);
        } else { // 若该顺序消费队列，缓存消息已被消费完，则立即请求Broker执行队列解锁
            this.unlock(mq, true);
        }
        return true;
    }
    ...
    public long computePullFromWhere(MessageQueue mq) {
        ...
        switch (consumeFromWhere) {
            ...
            case CONSUME_FROM_LAST_OFFSET: {
                // 若是广播模式则从本地读取最新消费进度，若是集群模式请求Broker获取最新消费进度
                long lastOffset = offsetStore.readOffset(mq, ReadOffsetType.READ_FROM_STORE);
                if (lastOffset >= 0) {
                    result = lastOffset;
                } else if (-1 == lastOffset) {  // First start, no offset
